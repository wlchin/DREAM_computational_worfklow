...
data <- data[which(!duplicated(data$Category) | data$Value == max(abs(data$Value), na.rm = TRUE)), ]
write(data$Category, "results/gene_blood_background_new_seed.txt")
...
ggplot(data, aes(x = Category, y = Value, fill = Value > 0)) +
  ...
  labs(title = "Average feature importance across 100 runs",
       x = "Category",
       y = "Value") +
  theme_minimal() +
  theme(
...
data <- read.csv("data/combined_weights.csv")
data <- data[,c(3,2)]
colnames(data) <- c("Category", "Value")
data <- data[data$Value != 0, ] # removed all non zero ones
...
ggsave("results/feature_plot_with_dashes.png", dpi = 600, width = 7.84, height = 3.07, units = "in")
